...
## Summary
- Reverted the AI listening-orb experiment changes introduced after the listening-stage investigation.
- Restored the AI orb preset routing so `#siri-orb` / `.ai-flow-orb-sphere` use the shared `pill` preset again instead of the smaller orb-specific preset.
- Restored the previous listening-orb CSS stack by removing the listening-only overrides that:
  - hid the orb shell glow
  - hid the refraction/blob layers
  - altered the reactive glow to remove its baseline
  - changed the compose-await listening-orb variant to the reduced stack
- This returns the listening orb code to the pre-experiment behavior, including the fuller white-ball appearance you referenced.
...
1. If you want to revisit the listening orb again, define the exact target stack first and then change only that stack with a browser-side check.

## Task title
Restore listening-orb fix state after rollback

## Completion status
- Completed

## Summary
- Restored the AI listening-orb fix state that had been rolled back.
- Reapplied the final listening-orb tweaks on top of the already-restored reduced shell stack:
  - hide the two image-highlight passes in listening mode
  - remove the strong static baseline from the reactive rim/highlight formulas
- Left the earlier listening-orb changes in place:
  - reduced listening-orb stack with static shell/refraction/blob layers hidden
  - smaller shared orb highlight preset routing
...
## Validation performed
- Reviewed the listening-orb selectors in `src/styles/ai-decorative.css`
...
## Recommended next step
1. Open `ai.html` and continue from the restored listening-orb fix state.

## Task title
Remove image highlights from AI listening orb

## Completion status
- Completed

## Summary
- Removed the two image-highlight layers from the AI listening orb.
- In listening mode, these layers are now hidden:
  - `.g-stage-selected-sharp-pass` (bottom-right highlight image pass)
  - `.g-stage-selected-highlight-mask` (top-left highlight image pass)
- Applied the same removal to `compose-await-orb.listening-orb` so the smaller listening-orb variant does not reintroduce the image highlights.
- Left the reactive rim and reactive full-surface highlight logic unchanged.
...
## Recommended next step
1. Open `ai.html` and verify the listening orb no longer shows the two image highlights, leaving only the reactive rim/highlight behavior.

## Task title
Remove static baseline from AI listening-orb reactive glow

## Completion status
- Completed

## Summary
- Updated the AI listening-orb reactive rim and inner-highlight formulas so they no longer keep a strong nonzero baseline at silence.
- Before this change, the listening orb still read as a bright white ball even after hiding the static shell/refraction layers, because the supposedly reactive glow started from visible base values.
- The listening reactive layers now scale directly from `--ai-listening-rim-level`:
  - accent rim opacity now follows volume
  - inner highlight opacity now follows volume
  - white glow intensity and spread now start near zero instead of from a visible default
- Kept the two highlight images visible, so the orb still has the localized highlight read even when the reactive glow is quiet.
...
## Validation performed
- Reviewed the listening-orb reactive selectors in `src/styles/ai-decorative.css`
- Confirmed the listening rim/highlight formulas now derive directly from `--ai-listening-rim-level` instead of adding a strong constant baseline
...
## Recommended next step
1. Open `ai.html` and verify the listening orb at low/no input now reads mostly as the two image highlights, with the white rim/glow building only as live volume increases.
