...
2. Give the four domain bubbles position-matched base diameters and lower their `y` positions so they sit closer to the larger bubbles below.
...
- Travel, Fitness, Budget, and Writing can all promote on hover-and-release in the `agent` set.
- The four domain bubbles use position-matched base diameters and are visibly closer to the lower row than before.
- Expanded top-level pills also push the home orb away instead of leaving it fixed underneath the pill body.
